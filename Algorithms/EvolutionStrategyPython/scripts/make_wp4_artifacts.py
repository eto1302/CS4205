...
def main():
    if len(sys.argv) < 2:
        print('Usage: make_wp4_artifacts.py <dest_dir>')
        sys.exit(1)
    dest = sys.argv[1]
    os.makedirs(dest, exist_ok=True)
    src_per = os.path.join('results', 'per_run.csv')
    src_cmp = os.path.join('results', 'comparisons.csv')
    dst_per = os.path.join(dest, 'per_run_wp4.csv')
    dst_cmp = os.path.join(dest, 'comparisons_wp4.csv')
    treatments = {'B+recomb_coord', 'B+recomb_pair'}
