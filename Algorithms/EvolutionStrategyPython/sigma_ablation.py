"""Sigma-strategy ablation: compare SINGLE vs MULTIPLE vs FULL variance strategies.
...
# Baseline: FULL_VARIANCE (as requested). Keep selection_scheme and init as in OFAT baseline.
BASELINE = dict(
...
def main():
    ...
    print(f"Sigma ablation: {len(TREATMENTS)} treatments x {len(CIRCLE_SIZES)} n x {N_SEEDS} seeds = {n_total} runs, budget {MAX_EVALS:,} evals\n")
